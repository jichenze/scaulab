...
public class AccessUtil {
	...
	public static boolean jugeInstanceAccess(String id) {
		AWebUserVO user = (AWebUserVO)ServletActionContext.getRequest().getSession().getAttribute(Constants.SESSION_USERVO);
		if(user.getUsertype().equals(Constants.DB_AGREEUSER_TYPE_0)){	
			//管理员
			return true;
		}else{
