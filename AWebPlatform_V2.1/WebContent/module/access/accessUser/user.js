define([ "jquery" ], function() {
    ...
    function initUserTb($el) {
        ...
        return $('#userTb', $el).dataTable({
            'sAjaxSource': './UserManagerAction_loadAllUser.do',
            'bDestroy': true,
            'bStateSave': true,
                        ...
                        if(data.status&&data.content){
                            for(items=data.content.aaData,i=items.length;(item=(items[--i]));) {
                                dealData.push([
                                    ...
                                    item.createTime,
                                    item.loginTime,
                                    item.state === "0" ? "未启用" : item.state === "1" ? "启用" : "锁定",
                                    item.remark,
                                ]);
                                userData[item.username] = item;
                            }
                        }else {
                            app.alert('用户信息', data.errorMsg || '加载用户信息错误', app.alertShowType.ERROR, app.alertMsgType.MESSAGE);
                        }
    ...
    function initUserRoleTb($el) {
        //关联角色表初始数据
        return $('#userRoleTb', $el).dataTable({
                    ...
                    success: function (data) {
                        ...
                        if(data.status&&data.content){
                            for(items=data.content.aaData,i=items.length;(item=(items[--i]));) {
                                if (item.state !== "0") {
                                    dealData.push([
                                        '<input type="checkbox" name="rolecheckbox" id="' + item.roleId + '"/>',
                                        item.name,
                                        item.createUser,
                                        item.createTime,
                                        item.updateTime,
                            ...
                            app.alert('角色信息', data.errorMsg || '加载角色信息错误', app.alertShowType.ERROR, app.alertMsgType.MESSAGE);
                        }
