...
(function ($) {
    ...
    function buildNode($node, $appendTo, level, opts) {
        var RUNNING=app.global.device.state.Running,
            NOOPR=app.global.device.action.dvcNoOpr,
            ISDTL=app.global.device.action.dvcIsDtl,
            NODEL=app.global.device.action.dvcNoDel,
            ACTION = app.global.device.action,
        ...
        if (opts.expand) {
            if ($childNodes.length) {
                $expandBtn = $("<span class='exp-col fa fa-minus-square' title='缩展'></span>");
                $nodeDiv.append($expandBtn);
                $expandBtn.click(function () {
                    var $expandBtn = $(this),
                        $tr = $expandBtn.closest("tr");

                    if ($expandBtn.hasClass('fa-minus-square')) {
        ...
        if (opts.control) {
            var $opciones = $nodeDiv.children('.opciones'),
                ...
                noDel=!!~className.indexOf(NODEL)||false,
                isDtl = !!(opts.isDtl ||~className.indexOf(ISDTL))|| false,
                html = '';
            ...
            if(className) {
                switch (className) {
                    ...
                    case 'ud':
                        //改删
                        html += getEditString(isDtl, isRun, noOpr,noDel);
                        break;
                    case 'c':
